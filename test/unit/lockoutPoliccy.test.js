...
describe('Edge Cases / Limit Cases', () => {

    it('si no se provee failedAttemptes, registerFaliure asume 0 y retorna 1', () => {
        const now = 1_000_000;
        const changes = registerFaliure({ now }); // Sin enviar failedAttempts
        expect(changes.failedAttempts).toBe(1);
        expect(changes.lockedUntil).toBeUndefined();
    });

    // Límite exacto de isLocked
    it('isLocked retorna false si lockedUntil es exactamente igual que now', () => {
        const now = 1_000_000;
